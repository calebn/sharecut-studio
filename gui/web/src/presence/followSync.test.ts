...
describe("followSync", () => {
  it("clamps remote playhead ghosts", () => {
    ...
    expect(remotePlayheadSec({ playhead_sec: 5 }, 0)).toBe(0);
    expect(remotePlayheadSec({ playhead_sec: 5, ...t(Number.NaN) }, 60)).toBe(
      5,
    );
    expect(remotePlayheadSec({ playhead_sec: 5, ...t(Infinity) }, 60)).toBe(5);
    expect(
      remotePlayheadSec({ playhead_sec: null, ...t(Number.NaN) }, 60),
    ).toBeNull();
