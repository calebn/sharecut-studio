...
| Browser compatibility | `e2e-compat/*.spec.ts` | Small cross-browser matrix for the playback control, phone layout, synthetic-media recording consent, and deep-zoom geometry at the 15 M px content ceiling (`e2e/deepZoom.ts`); runs Chromium and Playwright WebKit. Shared helpers: `e2e/recordRoom.ts` (record rooms, E2E flag, record links), `e2e/syntheticMicrophone.ts` (oscillator mic stub) and `e2e/scroll.ts` (`scrollToEnd`, re-applied until the end is reachable). Set `E2E_BRANDED_CHROME=1` locally to also use installed Chrome. |
