...
GitHub access: instructions below are written as \`gh\` commands, but this environment may not have the \`gh\` CLI installed, or it may lack permission for a given call even when installed — check for yourself rather than assuming. Try \`gh\` first; the moment any \`gh\` command errors (not found, 403, "not accessible by integration", auth failure, etc.), stop retrying \`gh\` for that operation and do the equivalent with the mcp__github__* MCP tools instead — do not skip the step or report it as blocked while an MCP path remains untried. Mapping for commands this workflow uses: \`gh issue edit --add-label/--remove-label\` and \`gh pr edit --add-label/--remove-label\` → mcp__github__issue_write (method update, labels: full desired array; PR numbers work too — GitHub treats a PR as an issue for labeling); \`gh issue view\`/\`gh pr view\` → mcp__github__issue_read / mcp__github__pull_request_read; \`gh issue list\`/\`gh pr list\`/\`gh api …/search\` → mcp__github__list_issues, mcp__github__list_pull_requests, mcp__github__search_issues, mcp__github__search_pull_requests; \`gh issue comment\`/\`gh pr comment\`/\`gh api …/comments\` (POST) → mcp__github__add_issue_comment (works on PR numbers too) or mcp__github__add_reply_to_pull_request_comment for a threaded reply; \`gh api -X PATCH …/comments/<id>\` → mcp__github__update_issue_comment; \`gh pr checks\`/\`gh api …/check-runs\` → mcp__github__pull_request_read (method get_check_runs or get_status); \`gh pr create\` → mcp__github__create_pull_request; \`gh pr merge\` → mcp__github__merge_pull_request; \`gh api graphql\` for review threads/replies/resolve → mcp__github__pull_request_read (method get_review_comments) to read, mcp__github__add_comment_to_pending_review / mcp__github__resolve_review_thread / mcp__github__unresolve_review_thread to write; \`gh issue create\` → mcp__github__issue_write (method create); \`gh run list\`/\`gh run view\`/\`gh run rerun\` → mcp__github__actions_list / mcp__github__actions_get / mcp__github__actions_run_trigger. For anything not listed here, search your available mcp__github__* tools for the closest match before giving up. Non-GitHub steps (git fetch/checkout/commit/push, local test commands, file edits) are unaffected — only the GitHub-hosted reads/writes need this fallback.`
