...
5. **Long jobs** that keep a workspace across slow work (pipeline steps, export) call `ws.checkpoint()` first and `ws.save_merged()` instead of `ws.save()`, so edits other requests commit meanwhile are merged, not overwritten.
