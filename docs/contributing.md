...
6. **Feedback**: Opus runs `/feedback` in plan mode and sorts every open item into `implement`, `follow_up` or `wont_do`. Sonnet then runs execute mode: it implements, files follow-up issues (added to the PR body as `Related #M`), and replies to and resolves every thread. Haiku verifies the replies. The script checks the plan against the PR's actual unresolved threads: missing threads trigger one re-plan that lists them, and made-up thread IDs are dropped. After execution, any thread still open (other than won't-dos) gets one more feedback pass before the gate. Steps 5–6 repeat up to `maxRounds`; the last round turns anything not trivially safe into a follow-up.
