...
When stage-specific subagents are available, the skill selects lighter models for bounded read-only work, Sol for ordinary implementation and the eight reviewer lenses, and Astra only for deeper unresolved judgment. A single Codex task cannot change its own model mid-run; the run report states which model choices and independent reviews were actually exercised. Model choice never relaxes CI or the deterministic merge gate.
