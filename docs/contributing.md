...
5. **Review**: a Sonnet agent builds one shared review packet per round: diff, changed code in context, callers, related tests and applicable repo rules. The script then runs `pr-multi-review`'s 8 reviewer lenses as parallel Sonnet agents (round 2+: 3 lenses on the fix diff only), because workflow agents cannot spawn their own subagents. Each lens gets the same packet followed by its own prompt, so the shared part is prompt-cached. Lenses run without worktrees and read extra code with `git show`. The packet also lists twin CLI/MCP/GUI paths and second-hop callers of service and domain functions. Lenses that need a wider view have required reading beyond it: reuse searches the whole repo for each new helper, security reads the authorization helpers and security docs, wiring checks every sibling adapter, concurrency traces threads, and patterns follows two hops. A lens whose concern has no surface in the packet returns empty with a reason. An Opus agent then runs the rest of the skill in AUTONOMOUS MODE with `lenses=supplied`: browser QA, merging the findings and posting them. Posting every finding is mandatory. A separate Haiku verifier re-posts anything missing, and the lane is held if a finding still cannot be posted.
