...
- **In-app cheatsheet:** modal via **?** or Transport **Menu → Keyboard shortcuts (?)** (`CommandPalette`) — category tabs; remaps optional. Prior art: Figma/Docs `?`, Premiere Help → Keyboard Shortcuts (not the VS Code command palette).
...
- **`EpisodeService`:** `add_track` (with media), `add_empty_track`, `set_track_media`, `set_track_meta`, `remove_track`, `reorder_track`; stale render via `record_invalidation` (not on reorder). Host MCP `track_add` / episode mutators call `notify_after_mutation`.
- **Document commands** (`AddTrack`, `SetTrackMedia`, `SetTrackMeta`, `RemoveTrack`, `ReorderTrack`) in `EDIT_COMMANDS` — host and share `edit` only (not `suggest` / `view`).
