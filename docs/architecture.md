...
`ProjectWorkspace.reload()` avoids a second JSON parse when the project's mtime, size, and file identity match its last load or commit. A changed file is loaded again; this is an in-process parse optimization, not a cross-process write lock.
