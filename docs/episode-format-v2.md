...
- Render path: clips → per-track WAV → mix → `artifacts/premix.wav` → master → `export/`. Stems don't bake the track gain; the mix applies each track's output gain. `artifacts/premix.hash` fingerprints the mix settings (track order, `muted`, `gain_db`, `fader_db`), so a volume or mute change reports `render_status.premix.stale_vs_mix` and `needs_rerender` without staling any stem.
