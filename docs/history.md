...
`run_mutation` compensates its own failure the same way. If `mutate`, the audio bookkeeping, `record(after)` or the commit raises (including a commit-lock timeout), it restores the in-memory `project.history` and, under `project_commit_lock`, puts `history/index.json` back to its pre-call payload and deletes the snapshots this call wrote (`project_store.rollback_own_history`), so the redo branch comes back and no dangling `before` entry is left. The history is kept when the commit landed (the project file revision changed and only a later write failed), when the project file cannot be stat'ed (only memory is restored), or when another writer recorded on top of this call's entries (a warning is logged). Cleanup failures are logged and the original error propagates. A failure inside `record(before)` itself is not compensated; an orphan snapshot is harmless. Restoring the rest of the in-memory editable state after a record/commit failure is #489.
