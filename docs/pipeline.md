...
9. `require_transcript_refine` — Hard agent gate (`refine-done` / waive; auto-waive with `--unattended` / `PODCAST_BATCH=1` when mode is `waive_unattended`)
...
20. `master_loudness` — two-pass loudnorm to podcast target; writes `artifacts/master_qc.json` verification report
21. `export_deliverables` — audio (WAV + configured FFmpeg formats), SRT, MD; writes `artifacts/export_qc.json` (reconciliation staleness + mastering QC rollup — check `ok` before shipping)
...
- **Analyze** proposes static knobs from diagnostics (hum, noise floor, gate, bleed, clipping); loudness measure→target still happens inside balance/master at run time
