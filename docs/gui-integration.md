...
**Zoom range:** 0.05 to 48,000 px/s, with a session-aware ceiling: content never passes `max_content_px` (15 M px), so an hour-long session stops near 4,167 px/s (`effectiveMaxZoomPxPerSec`, `clampZoomPxPerSec(zoom, sessionSec)`; [waveform.md § Deep zoom](waveform.md#deep-zoom)). A session-length change re-clamps around the view centre. Ruler steps run from 0.1 ms to 1 h (`niceTimeStep`, at least 70 px apart), and labels are `m:ss.f` to `m:ss.ffff` (`formatRulerTime`: `ceil(−log10 step)` decimals, 1–4), also used for the ruler's `aria-valuetext`. Only the 2048 px chunks that meet the viewport mount ticks, and the Levels envelope draws the same way, so deep zoom never builds millions of px of DOM or SVG.
