...
| **Render tiles** | `timeline/WaveformLayer.tsx` (memo, primitive props) splits a clip into 512-CSS-px tiles anchored to the media. Tile `k` covers media seconds `[k·512/zoom, (k+1)·512/zoom)`, so moving, trimming or splitting a clip only moves tiles and never re-rasterizes. Tiles mount for the view plus 512 px of overscan per side, from a selector that returns the tile range as a string. Each `canvas.clip-waveform-tile` covers only its part inside the clip, at paint DPR `min(round(dpr·8)/8, 2)`. The layer (`.clip-waveform`) covers the clip's border box, so waveform x matches ruler x. Clips narrower than 6 px draw no layer. |
...
| **Snap overlay** | The quiet wash comes from max-pooled pyramid columns at CSS-px resolution, at 8 px/s and above. Snap ticks come from `GET /api/waveform-snap` (`EditService.waveform_snap_window` → `preview_inaudible_cut` + windowed islands), around the dragged trim edge, else the blade hover inside the clip, else the paused playhead inside it (`hooks/useSnapTicks.ts`). They magnetize blade and trim. View-only guests get the wash only. Theme: `--color-waveform-quiet`, `--color-waveform-snap`. |
