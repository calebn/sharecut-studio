...
| Region | Content |
|--------|---------|
| Transport | Wide layout is three zones: project name; Play/Pause, Stop, timecode, and audition mode (Mix / FX / Raw) centered; render health, tools, Fit, a **View** menu (layers, zoom, theme, focus), and the project **Menu** (Project, Media, Help, with shortcuts) on the right. Collapsed widths keep one combined menu. The overflow uses menuitem, menuitemradio, and menuitemcheckbox semantics for keyboard and assistive-technology navigation. At compact widths every row, including audition radios and layer checkboxes, has `min-block-size: var(--touch-min)` (44px); browser coverage verifies their roles, selected state, arrow-key navigation, touch targets, and axe. |
