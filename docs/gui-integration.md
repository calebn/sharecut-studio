...
| **Per-track levels** | Stem and raw players play each track at its output gain (`gain_db + fader_db`). Media elements can't boost past volume 1, so when a track is boosted above 0 dB every track is lowered by that boost: the balance matches the mix. Players are rebuilt only when their sources change, so a volume or mute change applies while playing without stopping it. |
...
| **Volume** | Track inspector fader: saved `fader_db` (`track.setVolume` → `SetTrackFader`), −60 to +12 dB on top of the staging gain the pipeline's balance step writes (balance never touches it). Each step shows and plays at once; the last value of a burst is saved 300 ms after it (one undo step), with one send in flight per track. Double-click or **Reset** sets 0 dB. Read-only without `edit`, with the reason shown under it. The header gain strip shows the output gain. |
