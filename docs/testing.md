...
moved to its seek time), and history loading (`aria-busy` cleared; the last step row is reached). With a seeded history it then opens the last step's diff (`history-diff`) and runs one Undo and Redo (`history-undo-redo`). When the History list is virtualized it then tabs from the first step past the initially mounted rows and checks focus lands on the next step (`history-keyboard`). Undo and redo write to the generated project, so build a fresh one per run. A final `scrub-endurance` step re-opens the Transcript and runs `DAW_BENCHMARK_SCRUB_ROUNDS` rounds (default 20, a positive integer) of timeline scroll, Home/End, and 10 arrow-key seeks, printing a `large-project endurance:` JSON line of per-round DOM and post-GC heap samples. It is diagnostic only (no thresholds) and does not cover playback: the fixture WAVs are silent and headless autoplay is unreliable. Each step
