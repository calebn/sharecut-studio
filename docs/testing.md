...
The record-lobby room-tone E2E supplies quiet PCM through the guest page's real AudioWorkletNode message port. Headless macOS may render too few worklet quanta for a three-second sample before the capture deadline; the fixture keeps WAV persistence, upload, and consent gating under test without changing production capture behavior or increasing timeouts.
