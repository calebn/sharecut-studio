...
- **Busy:** when another process holds the project lock past 30 s (`filelock.Timeout`), or the `document.db` write lock stays busy past sqlite's busy timeout (`sqlite3.OperationalError` busy/locked), `POST /api/document/command` returns 503 with `X-Sharecut-Error-Code: project_busy` and the WS sends an `Error` frame with `code=project_busy` and stays open. Retry the same command (same `command_id` / `client_seq`). Undo/redo with `rerender` holds the project lock and the `document.db` write lock for its render; every `document.db` writer takes the project lock first, so other writers wait on the 30 s project lock, not the sqlite busy timeout. `Applied` is published inside both locks: `FanoutHub.publish` only schedules delivery (`call_soon_threadsafe`) and must stay non-blocking. Other adapters: #488.
