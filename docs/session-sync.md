...
**Viewport:** the time window a follower should show. A phone's fixed-playhead timeline scrolled before 0 publishes `[0, span]`, keeping the leader's span so followers keep its zoom, which can be wider than what is on that screen (`zoomScrollToViewport`). The span is never under `MIN_VIEWPORT_SPAN_SEC` (0.001 s), so a near-sample zoom (up to 48,000 px/s) still publishes its window; the server rejects anything shorter.
