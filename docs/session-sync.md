...
| Chunk ACK | `record_upload_parts` / `record_upload_files` in this DB (`join_offset_ms`, `clipping_regions` JSON `[[start_ms,end_ms],...]` from the final part, `landed_ns`); part files under `artifacts/record/`; landing copies ACK'd WAV into `raw/` + clips. |
