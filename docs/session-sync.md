...
**Playhead validation.** The top-level `playhead_sec` on Ack, `PresenceHeartbeat`, and `FollowUser` is validated on the server with the same rule as `meta.transport.playhead_sec` (finite, `>= 0`; `normalize_presence_playhead`). An invalid value is dropped, not rejected: the rest of the frame applies, a guest frame is not counted as malformed, and the last good playhead is kept. Clamping to the session length is done by the receiver (the overlay), since a socket's loaded project can go stale after edits.
