...
Host document WebSocket commands submit in a worker thread so project parsing and mutation do not block the socket event loop. `ProjectWorkspace.reload()` checks the project file's mtime, size, and file identity under the existing submit lock; it reuses the loaded project when unchanged and reads it again after an external write. Workspace save/mutate paths refresh that signature after committing.
