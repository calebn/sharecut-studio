...
| Session sync / document log | **sqlite** (`session_sync/`) | DAW command authority | Extend `services/session_sync/`; do not invent parallel logs. Snapshot read-modify-writes (`append_and_apply`, `mutate_snapshot`, `reset`) are one `BEGIN IMMEDIATE` transaction for every table prefix. Presence (`clients.meta`) is ephemeral in the same DB and is never journaled. All stores sharing `artifacts/session/sync.db` use the shared connection initializer, which serializes WAL setup and skips switching modes when already in WAL before schema setup. |
