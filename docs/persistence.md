...
|-------|------------|------|------------|
| Episode project + history | JSON + history snapshots | Editorial source of truth; snapshots publish before the atomically replaced `history/index.json` | Commits, history index writes and undo/redo/goto take `util.project_state.project_commit_lock` (in-process lock plus a file lock at `artifacts/episode.project.json.lock`, 30 s timeout) | Use `ProjectWorkspace.mutate` / services |
