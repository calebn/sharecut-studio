...
- **One measurement.** The scroller's time viewport (width minus the header column) sets the fit, the lead pads, the line (`--timeline-fixed-line`) and the center math. A classic scrollbar's gutter therefore can't shift the line. Fixed-playhead scrollers reserve no empty gutter, because Chromium shortens the scroll range by the gutter width and the end became unreachable.
- **Only a person's scroll seeks.** Fit, recentering and follow writes are programmatic; a late scroll event that lands on the last written position is their echo; and a move under one pixel (`PLAYHEAD_MOVE_MIN_PX`) never changes the time. Scrubbing stops at the session end.
- **Zoom.** Fit, menu and keyboard zoom anchor at the line and keep the playhead. Pinch and Ctrl+wheel, claimed anywhere on the timeline except the track headers, keep the time under the fingers still, and the playhead takes the new center time.
