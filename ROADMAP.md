...
| **Lobby / consent / host transport** | **Shipped** (`feat/recording-lobby-consent`): lobby (name, headphones, own-mic peak meter with clip LED; sample peak only, [#174](https://github.com/calebn/sharecut-studio/issues/174)), per-person consent, host Start/Pause/Resume/Stop, REC/PAUSED clock, roster with **Not recorded**, full-room/declined pages, CLI/MCP twins. Host admit / waiting room is [Follow-up](#follow-up). |
...
| **Room-tone at record time** | **Shipped** (`feat/record-room-tone-bed`): optional 3 s quiet bed in the lobby (keeper constraints, skippable); lands as `track.room_tone` / `raw/room-tone/{session}/{participant}.wav` (the older `raw/room-tone/{participant}.wav` still resolves for existing projects); `filler_pad_mode: room_tone` prefers the bed over stolen air. Default pad mode stays silence. Pair with **Find room-tone candidates** (Follow-up) as offline fallback. |
...
| **Large-project performance ([#29](https://github.com/calebn/sharecut-studio/issues/29))** | Shipped: opt-in benchmark ([docs/testing.md](docs/testing.md) § Large-project browser profile), transcript virtualization, history-list virtualization with a seeded 1,000-entry history, and an extended scrub-session memory profile. Follow-ups: timeline clip windowing, and playback-length memory profiling. |
