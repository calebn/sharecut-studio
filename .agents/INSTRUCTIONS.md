...
4. **Git workflow:** work on a feature branch off latest `main`, not on `main` itself. Branch names: `type/short-kebab-description` (`feat` | `fix` | `docs` | `chore` | `refactor` | `test`). When the user asks to ship, commit on that branch, `git push -u origin HEAD`, and open a PR to `main` with `gh pr create`. Put `Fixes #N` for issues the PR closes and `Related #N` for issues it only touches. Do not push commits to `main` or merge the PR unless the user explicitly asks (the `issue-pipeline` workflow is pre-approved to merge when its gate passes). Detail: [docs/contributing.md § Git workflow](../docs/contributing.md#git-workflow).
...
Skills under `skills/` describe **how to operate** the toolkit on episode workspaces via MCP or CLI, plus the repo-specific `codex-issue-pipeline` contributor workflow. They do not replace the repo’s code standards. See [README.md](README.md) for MCP registration on any client.
