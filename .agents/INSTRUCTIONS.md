...
3. Run `make test` before proposing a commit.
4. **Git workflow:** work on a feature branch off latest `main`, not on `main` itself. Branch names: `type/short-kebab-description` (`feat` | `fix` | `docs` | `chore` | `refactor` | `test`). When the user asks to ship, commit on that branch, `git push -u origin HEAD`, and open a PR to `main` with `gh pr create`. Do not push commits to `main` or merge the PR unless the user explicitly asks. Detail: [docs/contributing.md § Git workflow](../docs/contributing.md#git-workflow).
