...
| Near-sample zoom (#429) | Zoom now runs to 48,000 px/s (about a pixel per sample at 48 kHz), capped per session so a long episode stays under 15 M px of timeline. Hosts see individual samples drawn as a line; guests stop at pyramid resolution. The ruler labels to the millisecond (`m:ss.fff`) at deep zoom and always shows at least one decimal. To review: label density and legibility at every zoom (labels now read `0:02.0` when zoomed out), how the deep-zoom line reads in both themes, and whether guests need a cue that they are at their resolution limit. |
